 ...
 * primeiro — é também a ordem de consumo (FIFO por validade). */
export async function getRecargasAtivas(
...
export async function getSaldoCreditos(
  ...
  const recargas = await getRecargasAtivas(supabase, userId);
 ...
 *
 * O update é condicionado ao valor de creditos_restantes lido (compare-and-
 * swap): se duas requisições concorrentes (ex: duplo clique, duas abas)
 * lerem o mesmo saldo e tentarem consumir o último crédito ao mesmo tempo,
 * só uma delas bate a condição e consome de verdade — a outra tenta de novo
 * com o saldo atualizado, em vez de as duas debitarem o mesmo crédito. */
...
/** Registra uma recarga comprada (chamada pelo webhook do Stripe, após
 * pagamento confirmado). Idempotente pelo id da sessão de checkout — uma
 * mesma sessão nunca concede crédito duas vezes. */
export async function registrarRecarga(params: {
  ...
  if (!pacote) {
    console.error(`[creditos] pacote desconhecido: ${params.pacoteId}`);
    return;
  }
  ...
  if (error && error.code !== "23505") {
    ...
    console.error(`[creditos] falha ao registrar recarga: ${error.message}`);
  }
}
