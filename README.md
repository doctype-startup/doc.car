...
6. **PIX na compra de recarga**: `app/api/checkout-creditos` já pede `payment_method_types: ["card", "pix"]` — mas o Stripe só oferece PIX de fato se ele estiver **ativado pra sua conta**. Confirme em **Settings > Payment methods** que PIX aparece como "On" (contas brasileiras novas costumam já vir habilitadas; se não vier, tem que ativar manualmente ali). Sem isso, o checkout cai só no cartão. A assinatura mensal (`app/api/checkout`) continua só cartão — PIX não é reutilizável pra cobrança automática recorrente.

### 3. Dados reais de veículo

1. Usa a API Brasil (`gateway.apibrasil.io`, produto `endereco-telefone-por-placa`) — mesmo token das outras consultas desse provedor → `APIBRASIL_TOKEN`. Não precisa de variável própria.
2. `lib/dados-veiculo.ts` chama esse provedor e devolve ficha do veículo (placa, chassi, Renavam, marca/modelo, ano, cor, combustível) e o dossiê completo do proprietário atual.
3. **Decisão de produto — dados pessoais na consulta simples**: diferente da versão anterior deste arquivo (que só liberava o nome do proprietário, por LGPD), essa consulta agora traz o dossiê pessoal completo devolvido pelo provedor: nome, CPF/CNPJ, nome da mãe, data de nascimento/fundação, situação na Receita Federal, telefones (celular e fixo), e-mails e endereços. **Sem reconfirmação do CPF/CNPJ do despachante nem auditoria própria** — diferente da aba CRM (`app/api/crm/proprietario`), que expõe dado pessoal parecido atrás desses dois controles. Foi uma escolha deliberada do produto; revise as implicações de LGPD antes de mudar esse comportamento de novo.
